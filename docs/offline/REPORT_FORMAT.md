...
Brain reports additionally include model ID, backend, planner call count, decision intent, decision confidence, validation errors, loop state transitions, stop reason, selected recipe, verifier results, and rollback command. Raw Qwen output is stored only after redaction.
