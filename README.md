...
> **Working repo — the live branch is `agentlink-rescue-by-opus`.** The body below documents the v0.5.2 GUI field-beta base (the shared CLI core reports `0.5.1`). The live branch adds, on top of that base, the **Opus kernel refactor** — the distilled knowledge is now wired into the execution kernel (`diagnose → classify → diagnosisgraph → planner → recipe/repair → verifier → safety`, no orphan classes) — plus the connectivity-first **`nuclear`** repair level. Start with **[VISION.md](VISION.md)** (the human vision + the judgment calls that must not drift), then **[HANDOFF.md](HANDOFF.md)** for current-state orientation. Authoritative current-state docs: **[HANDOFF.md](HANDOFF.md)**, `governor/reports/AGENTLINK_KERNEL_OPUS_REFACTOR_SCORECARD.md`, and `governor/reports/AGENTLINK_KERNEL_ARCHITECTURE_REVIEW.md`. The clean public snapshot of this branch is the separate repo `cactusaudio/AgentLink-Rescue-by-Opus`.
