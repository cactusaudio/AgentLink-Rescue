...
OUT_ROOT="${OUT_ROOT:-$HOME/CactusLocalAgent/.asset-cache/AgentLink-Rescue/release-artifacts/gpt-pro-stress-lite-$STAMP}"
PKG_ROOT="$OUT_ROOT/AgentLink-Rescue"
ZIP_PATH="$OUT_ROOT/agentlink-gpt-pro-prerelease-stress-lite-$STAMP.zip"
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "python3 is required to package GPT Pro stress artifacts" >&2
  exit 2
fi
if ! command -v rsync >/dev/null 2>&1; then
  echo "rsync is required to package GPT Pro stress artifacts" >&2
  exit 2
fi
if ! command -v zip >/dev/null 2>&1; then
  echo "zip is required to package GPT Pro stress artifacts" >&2
  exit 2
fi
...
manifest = {
    ...
    "stamp": stamp,
    "package": "agentlink-gpt-pro-prerelease-stress-lite",
    ...
    "intent": "GPT Pro cloud fixture/read-only/dry-run stress package; no model weights, runtimes, DMGs, or release archives.",
    "expectedEntryPoint": "FULL=1 bash scripts/gpt_pro_stress_test.sh",
    "host": {"system": platform.system(), "machine": platform.machine()},
    "excluded": ["*.gguf", "*.safetensors", "*.dmg", "*.zip", "*.tar.gz", ".git", ".gpt-pro-build", "gpt-pro-results", "node_modules", ".build"],
}
