...
func TestBrainPackageScriptRequiresAssetsAndHygiene(t *testing.T) {
	...
	text := string(data)
	for _, want := range []string{"Cactus-AgentLink-Rescue-v0.4.5-brain-gemma4-e4b-q4km.zip", "DOWNLOAD=1", "assets/models/gemma-4-E4B-it-Q4_K_M.gguf", "assets/runtimes/llama.cpp", "assets/installers", "manifest.lock.json", "COPYFILE_DISABLE=1", "zip -r -X"} {
		if !strings.Contains(text, want) {
