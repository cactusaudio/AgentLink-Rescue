...
func TestPackageScriptRemovesMetadataFiles(t *testing.T) {
	...
	for _, want := range []string{"COPYFILE_DISABLE=1", "zip -r -X", "unzip -l", "AppleDouble", "/\\._", "Cactus-AgentLink-Rescue-v0.5.0-core.zip", "assets/manifests", "assets/installers", "fetch_clash_verge_rev.sh", "core package must not contain GGUF", "core package must not contain llama.cpp"} {
		if !strings.Contains(text, want) {
...
func TestBrainPackageScriptRequiresAssetsAndHygiene(t *testing.T) {
	...
	text := string(data)
	for _, want := range []string{"Cactus-AgentLink-Rescue-v0.5.0-brain-gemma4-e4b-q4km.zip", "DOWNLOAD=1", "resolve_model_path", "AGENTLINK_ASSET_CACHE", "copy_model_to_staging", "copy_runtime_to_staging", "assets/runtimes/llama.cpp", "assets/installers", "manifest.lock.json", "COPYFILE_DISABLE=1", "zip -r -X"} {
		if !strings.Contains(text, want) {
...
func TestMacBookFieldPackageScriptEmbedsFieldMode(t *testing.T) {
	...
	text := string(data)
	for _, want := range []string{"Cactus-AgentLink-Rescue-v0.5.0-macbook-field-gui-proxykit.zip", "Cactus MacBook Network Rescue", "field-mode.json", "RUN-FIRST.command", "README-MACBOOK-NETWORK-RESCUE.txt", "emergency-terminal-commands.txt", "COPYFILE_DISABLE=1", "zip -r -X"} {
		if !strings.Contains(text, want) {
