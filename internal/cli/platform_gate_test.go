...
func TestNonDarwinCloudAllowed(t *testing.T) {
	allowed := []struct {
		...
		{"guided", []string{"rescue", "--target", "auto", "--dry-run", "--json"}},
		{"guided", []string{"rescue", "--target=proxy", "--dry-run", "--json"}},
...
func TestNonDarwinCloudDenied(t *testing.T) {
	denied := []struct {
		...
		{"guided", []string{"rescue", "--target", "auto", "--json"}},
		{"guided", []string{"rescue", "--yes", "--json"}},
		{"guided", []string{"rescue", "--target", "auto", "--dry-run", "--yes", "--json"}},
